...
building_area_agg <- df %>% 
  mutate(B_Area = case_when(
    `Building Area(㎡)` <= 500 ~ "0-500",
    `Building Area(㎡)` > 500 & `Building Area(㎡)` <= 1000 ~ "500-1000",
    `Building Area(㎡)` > 1000 & `Building Area(㎡)` <= 1500 ~ "1000-1500",
    `Building Area(㎡)` > 1500 & `Building Area(㎡)` <= 2000 ~ "1500-2000",
    `Building Area(㎡)` > 2000 & `Building Area(㎡)` <= 2500 ~ "2000-2500",
    `Building Area(㎡)` > 2500 & `Building Area(㎡)` <= 3000 ~ "2500-3000",
    `Building Area(㎡)` > 3000 & `Building Area(㎡)` <= 3500 ~ "3000-3500",
    `Building Area(㎡)` > 3500 ~ "3500+",
  ))

building_area_agg$B_Area <- factor(building_area_agg$B_Area, levels=c("0-500", "500-1000", "1000-1500", "1500-2000", "2000-2500", "2500-3000","3000-3500","3500+"), ordered = TRUE)
building_area_agg %>%
  group_by(B_Area) %>% 
  summarise(Price = mean(`Price (1000won)`) * 1000) %>% 
  ...
  geom_text(aes(x= B_Area, y= Price, label = paste(round(Price/1000000,0), 'm')), position = position_dodge(width = 1),vjust = -0.5, size = 4,check_overlap = T) +
  theme(axis.text.x = element_text(angle= 30, vjust = 0.5,size =8))
```
